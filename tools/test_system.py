...
class SystemTelemetryTests(unittest.TestCase):
    def test_process_cpu_uses_interval_not_lifetime(self):
        ...
        after = {"7": ("worker", 10000 + system.RELOJ, 80, 42)}
        self.assertEqual(system.top(before, after, 2)[0]["cpu"], 50)

    def test_multithread_cpu_can_exceed_100(self):
        self.assertEqual(system.top({"7": ("a", 0, 0, 1)},
                                   {"7": ("a", 4 * system.RELOJ, 0, 1)}, 2)[0]["cpu"], 200)

    def test_pid_reuse_and_counter_reset_are_not_activity(self):
        self.assertEqual(system.top({"7": ("a", 100, 50, 1)}, {"7": ("b", 900, 50, 2)}, 2), [])
        self.assertEqual(system.top({"7": ("a", 100, 50, 1)}, {"7": ("a", 20, 50, 1)}, 2), [])

    def test_invalid_or_suspended_interval(self):
        for dt in (0, -1, 20):
            self.assertEqual(system.top({"7": ("a", 0, 50, 1)}, {"7": ("a", 900, 50, 1)}, dt), [])
    ...
    def test_memory_leaders_survive_cpu_ranking(self):
        ...
        self.assertEqual({r["pid"] for r in system.top(before, after, 2, 1)}, {1, 9})

    def test_process_stat_parentheses_and_real_page_size(self):
        fields = ["0"] * 22
        fields[11], fields[12], fields[19], fields[21] = "12", "8", "90", "64"
        with patch.object(system, "PAGE_SIZE", 65536):
            self.assertEqual(system.parse_process("7 (a (worker)) " + " ".join(fields)),
                             ("a (worker)", 20, 4, 90))

    def test_filesystem_preserves_reserved_space(self):
        data = os.statvfs_result((4096, 4096, 1000, 200, 150, 0, 0, 0, 0, 255))
        with patch("system.os.statvfs", return_value=data):
            result = system.storage("/home/example")
    ...
    def test_missing_filesystem(self):
        with patch("system.os.statvfs", side_effect=OSError):
            self.assertIsNone(system.storage("/missing"))
    ...
    def test_filesystem_identity_uses_longest_mount_and_decodes_spaces(self):
        ...
        identity = system.filesystem_identity("/home/example data/files", mounts)
    ...
    def test_missing_sensors(self):
        with tempfile.TemporaryDirectory() as directory:
            self.assertEqual(system.rutas_temperatura(Path(directory)), {"cpu": "", "nvme": ""})
    ...
    def test_amd_gpu_available_and_missing_counters(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            gpu = system.discover_gpu(Path(directory))
            reading = system.gpu_reading(gpu)
            ...
            self.assertIsNone(system.gpu_reading(gpu)["usage"])

    def test_nvidia_unavailable_value_is_not_zero(self):
        result = type("Result", (), {"returncode": 0, "stdout": "Example GPU, [N/A], 50, 128, 4096\n"})()
        with patch("system.subprocess.run", return_value=result):
            reading = system.gpu_reading({"provider": "nvidia"})
    ...
    def test_end_process_checks_identity_before_signalling(self):
        with patch("system.os.pidfd_open", return_value=8), patch("system.os.close"), \
                patch("system.Path.read_text", return_value="ignored"), \
                patch("system.parse_process", return_value=("worker", 0, 0, 43)), \
                patch("system.signal.pidfd_send_signal") as send:
            with self.assertRaises(ProcessLookupError):
                system.terminate(7, "42")
            send.assert_not_called()

    def test_terminate_own_fixture_process(self):
        child = subprocess.Popen(["python3", "-c", "import time; time.sleep(30)"])
        try:
            start = system.parse_process(Path(f"/proc/{child.pid}/stat").read_text())[3]
            system.terminate(child.pid, str(start))
